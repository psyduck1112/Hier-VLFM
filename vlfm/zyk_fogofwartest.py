...
def get_two_farthest_points(source: np.ndarray, cnt: np.ndarray, agent_yaw: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    rotation_matrix = np.array([[np.cos(-agent_yaw), -np.sin(-agent_yaw)], [np.sin(-agent_yaw), np.cos(-agent_yaw)]])
...
def reveal_fog_of_war_with_visualization(
    ...
    # 步骤4: 查找障碍物轮廓
    obstacle_contours, _ = cv2.findContours(obstacles_in_cone, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # 步骤7: 查找最终轮廓
    final_contours, _ = cv2.findContours(visible_cone_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for i, cnt in enumerate(final_contours):
        ...
        if M["m00"] != 0:
            cx = int(M["m10"] / M["m00"])
            cy = int(M["m01"] / M["m00"])
            cv2.putText(vis_final_contours, f"{i}", (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
