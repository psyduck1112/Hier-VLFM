...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.running_mean_and_var = (
            nn.Sequential()
        )  # 创建一个空的 Sequential 容器，用于可能的均值和方差归一化处理，但当前为空
...
class PointNavResNetNet(nn.Module):
    def __init__(
        self, discrete_actions: bool = False, no_fwd_dict: bool = False
    ):  # discrete_actions: 是否使用离散动作空间 no_fwd_dict: 是否在前向传播中返回额外字典
        super().__init__()
        if discrete_actions:  # 如果是离散动作空间
            self.prev_action_embedding_discrete = nn.Embedding(
                4 + 1, 32
            )  # 使用嵌入层将离散动作（4种基本动作+1个起始标记）映射到32维向量
        else:
            self.prev_action_embedding_cont = nn.Linear(
                in_features=2, out_features=32, bias=True
            )  # 连续动作：使用线性层将2维连续动作映射到32维向量
