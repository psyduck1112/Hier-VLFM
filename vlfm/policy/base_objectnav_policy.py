...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__( # 初始化所有子模块
        ...
        **kwargs: Any,
        ...
        self._object_map: ObjectPointCloudMap = ObjectPointCloudMap(erosion_size=object_map_erosion_size) # 存储目标点云
    ...
    def act(
        ...
        detections = [ # 循环更新物体地图，返回每帧检测结果
            self._update_object_map(rgb, depth, tf, min_depth, max_depth, fx, fy) # 对 object_map_rgbd 的每一帧图像进行循环处理
        ...
        action_numpy = pointnav_action.detach().cpu().numpy()[0] # 先分离计算图，不再计算梯度，再移到 CPU，再转成 NumPy 数组
    ...
    def _pre_step(self, observations: "TensorDict", masks: Tensor) -> None:
        ...
        except IndexError as e: # 如果缓存过程中发生索引错误（通常是到达地图边界），打印错误信息并抛出StopIteration异常来停止当前 episode
            raise StopIteration
    ...
    def _get_object_detections(self, img: np.ndarray) -> ObjectDetections:
        ...
        has_coco = any(c in COCO_CLASSES for c in target_classes) and self._load_yolo # 判断是否有目标类别在coco内，同时yolo是否已加载
        ...
        det_conf_threshold = self._coco_threshold if has_coco else self._non_coco_threshold # 根据检测器类型不同设置不同阈值
    ...
    def _pointnav(self, goal: np.ndarray, stop: bool = False) -> Tensor:
        ...
        """
        masks = torch.tensor([self._num_steps != 0], dtype=torch.bool, device="cuda") # 如果不是第一步，就设成 True，否则是 False
        ...
        rho, theta = rho_theta(robot_xy, heading, goal) # 得到机器人和目标点之间的距离，目标点相对于机器人正前方的角度偏差
        rho_theta_tensor = torch.tensor([[rho, theta]], device="cuda", dtype=torch.float32) # 把极坐标(rho, theta) 转成张量形式，放到 GPU 上
        ...
        action = self._pointnav_policy.act(obs_pointnav, masks, deterministic=True) # 调用pointnav策略（使用模型生成动作）
    ...
    def _update_object_map(
        ...
        if np.array_equal(depth, np.ones_like(depth)) and detections.num_detections > 0: # 如果深度全为1（即无效深度值），且检测到物体
