...
class JetRacerROSMixin:
    ...
    def _setup_subscribers(self) -> None:
        ...
        self._rgb_sub = rospy.Subscriber(
            "/camera/color/image_raw", RosImage, self._rgb_callback, queue_size=1, buff_size=2**24
        )

        # 深度图像 - 使用矫正后的深度图像
        self._depth_sub = rospy.Subscriber(
            "/camera/depth/image_rect_raw", RosImage, self._depth_callback, queue_size=1, buff_size=2**24
        )
    ...
    def _odom_callback(self, msg: Odometry) -> None:
        ...
        try:
            ...
            with self._data_lock:
                self._current_odom = {"x": position.x, "y": position.y, "yaw": yaw}
    ...
    def _cache_observations(self: Union["JetRacerROSMixin", ITMPolicyV2], observations: Dict[str, Any]) -> None:
        ...
        with self._data_lock:
            # 获取最新传感器数据
            rgb_image = (
                self._current_rgb.copy() if self._current_rgb is not None else np.zeros((480, 640, 3), dtype=np.uint8)
            )
            depth_image = (
                self._current_depth.copy()
            ...
            if rgb_image is not None:
                rospy.loginfo_throttle(
                    5, f"🔄 缓存更新: RGB均值={rgb_image.mean():.1f}, 位置=({robot_xy[0]:.2f}, {robot_xy[1]:.2f})"
                )
        ...
        print("🔧 JetRacer 非Habitat PointNav调试:")
        ...
        self._observations_cache = {
            ...
            "object_map_rgbd": [(rgb_image, depth_image, tf_matrix, min_depth, max_depth, fx, fy)],
            # 实机策略格式：value_map_rgbd包含6元组列表
            "value_map_rgbd": [(rgb_image, depth_image, tf_matrix, min_depth, max_depth, topdown_fov)],
        }
