...
class JetRacerVLFMVisualizer:
    ...
    def collect_data(
        ...
        annotated_rgb = self._overlay_status_info(annotated_rgb, action_info, target_object, cosine_similarity)
        ...
        map_vis = self._create_map_visualization(obstacle_map, explored_area, frontiers, robot_xy, robot_heading)
            ...
            try:
                ...
                if h > 0 and w > 0:
                    resized_obs = cv2.resize(
                        obs_T.astype(np.uint8), (self.map_size, self.map_size), interpolation=cv2.INTER_NEAREST
                    )
                    resized_exp = cv2.resize(
                        exp_T.astype(np.uint8), (self.map_size, self.map_size), interpolation=cv2.INTER_NEAREST
                    )
            ...
            try:
                ...
                for i, (px_x, px_y) in enumerate(frontier_px):
                    ...
                    if 0 <= px_x < self.map_size and 0 <= px_y < self.map_size:
                        ...
                        # 添加点标号便于调试
                        cv2.putText(
                            map_vis, str(i), (px_x + 8, px_y - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1
                        )
                    else:
        ...
        if len(self.trajectory_points) > 1:
            try:
                ...
                for i in range(1, len(traj_px)):
                    ...
                    if (
                        0 <= x1 < self.map_size
                        and 0 <= y1 < self.map_size
                        and 0 <= x2 < self.map_size
                        and 0 <= y2 < self.map_size
                    ):
                        cv2.line(map_vis, (x1, y1), (x2, y2), self.colors["trajectory"], 2)
            except Exception as e:
    ...
    def _create_obstacle_map_vis(
        self, obstacle_map: np.ndarray, robot_xy: np.ndarray, robot_heading: float
    ) -> np.ndarray:
        """
        创建障碍物地图的专门可视化
    ...
    def _overlay_status_info(
        self, image: np.ndarray, action_info: Dict[str, Any], target_object: str, cosine_similarity: Optional[float]
    ) -> np.ndarray:
        """
        在图像上叠加状态信息
    ...
    def _create_combined_frame(
        ...
        try:
            ...
            left_panel_resized = cv2.resize(
                left_panel, (int(left_panel.shape[1] * target_h / left_panel.shape[0]), target_h)
            )
            right_panel_resized = cv2.resize(
                right_panel, (int(right_panel.shape[1] * target_h / right_panel.shape[0]), target_h)
            )
    ...
    def _add_text_overlay(self, image: np.ndarray, text: str, top: bool = False) -> np.ndarray:
        ...
        cv2.putText(text_img, text, (10, text_size[1] + 10), font, font_scale, (0, 0, 0), font_thickness)
    ...
    def _pad_frames(self, frames: List[np.ndarray]) -> List[np.ndarray]:
        ...
        for frame in frames:
            ...
            padded_frame = np.pad(
                frame, ((0, height_diff), (0, width_diff), (0, 0)), mode="constant", constant_values=255
            )
...
class JetRacerVideoRecorder:
    ...
    def save_episode_video(
        self, frames: List[np.ndarray], episode_id: Optional[str] = None, metrics: Optional[Dict[str, float]] = None
    ) -> str:
        """
        保存episode视频
