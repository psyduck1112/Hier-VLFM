...
def pixel_value_within_radius(
    ...
    """在图像中指定像素点周围的一个圆形区域内，计算该区域内像素值的统计值（如中位数、平均值或最大值）。
    ...
    # 计算以指定像素为中心、给定半径为范围的矩形区域的边界坐标，确保不超出图像边界
