...
def contour_to_frontiers(contour, unexplored_mask):
    ...
    # 步骤3：检测是否为环形轮廓
    front_last_split = 0 not in bad_inds and len(bad_inds) > 0 and max(bad_inds) < num_contour_points - 2
    print("\n步骤3：环形轮廓检测")
...
"""
    ...
    ax3.text(0.1, 0.9, info_text, transform=ax3.transAxes, fontsize=12, verticalalignment="top", fontfamily="monospace")
