...
def add_text_to_image(image, text, position=(10, 30)):
    ...
    # 添加文字
    cv2.putText(img_with_text, text, position, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(img_with_text, text, position, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
...
def visualize_depth_processing_steps(depth):
    ...
    # 添加说明文字
    cv2.putText(large_img, "Step 2: Max depth per column", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(
        large_img,
        "Each column shows max depth value from original image",
        (10, 140),
    ...
    # 添加轴标签
    cv2.putText(coord_vis, "X (depth direction)", (450, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    cv2.putText(coord_vis, "Y (lateral offset)", (310, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    ...
    for i in range(0, len(x_coords), 2):  # 每2个点绘制一个
        ...
        if 0 <= img_x < 600 and 50 <= img_y < 250:
            ...
            if i % 20 == 0:
                label = f"{x:.2f}"
                cv2.putText(coord_vis, label, (img_x - 15, img_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 200, 255), 1)

    # 添加说明文字
    cv2.putText(
        coord_vis, "Step 3: 3D coordinate calculation", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
    )
    cv2.putText(
        coord_vis,
        f"FOV: {np.rad2deg(fov):.1f} degrees, Points plotted: {points_drawn}",
        (10, 300),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (200, 200, 200),
    ...
    cv2.putText(
        coord_vis,
        "X: depth (front-back), Y: lateral offset (left-right)",
        (10, 320),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (200, 200, 200),
    ...
    # 添加说明文字
    cv2.putText(
        pixel_vis, "Step 4: Pixel coordinate conversion", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
    )
    cv2.putText(
        pixel_vis,
        f"Pixels per meter: {value_map.pixels_per_meter}",
        (10, 280),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (200, 200, 200),
    ...
    cv2.putText(
        pixel_vis, f"Points plotted: {points_drawn}", (10, 300), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1
    )
