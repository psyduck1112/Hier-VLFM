...
from vlfm.utils.jetracer_vlfm_visualizer import JetRacerVideoRecorder, JetRacerVLFMVisualizer  # VLFM风格可视化
...
class VLFMJetRacerController:
    ...
    def _initialize_vlfm(self):
        ...
        try:
            ...
            rospy.loginfo(
                f"[ObstacleMap] kernel_size={self.obstacle_map._navigable_kernel.shape[0]}, "
    ...
    def _run_vlfm_step(self) -> Optional[Twist]:
        ...
        if hasattr(self, "_imu_offset"):
            # 显示和VLFM内部都使用归零后的坐标
            robot_xy = np.array([raw_robot_xy[0] - self._imu_offset["x"], raw_robot_xy[1] - self._imu_offset["y"]])
        ...
        try:
            ...
            rospy.loginfo(
                f"[{policy_mode}]{status_str} Action: linear={cmd.linear.x:.3f},"
                f" angular={cmd.angular.z:.3f}{cos_str}{value_str}"
            )
            ...
            # 即使出错也尝试收集可视化数据
            if self.enable_visualization and self._current_rgb is not None and self._current_depth is not None:
                try:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and not sensor_ready and wait_count < 50:  # 最多等待10秒
            if self._current_rgb is not None and self._current_depth is not None and self._last_odom_timestamp > 0.0:
                sensor_ready = True
                ...
                rospy.loginfo("项目运行起始位置已重置为: (0.000, 0.000), yaw=0.0°")
                rospy.loginfo("🚀 IMU参考原点设置完成")

                # 取消强制地图中心设置，使用BaseMap的默认逻辑
                # IMU已经归零，直接使用原始坐标即可
                rospy.loginfo("📍 使用BaseMap默认地图中心，IMU已归零")
                rospy.loginfo("使用标准地图坐标系，开始绘制地图")
            else:
                wait_count += 1
                rospy.sleep(0.2)
    ...
    def _update_vlfm_visualization(
        self, action_result: Dict[str, Any], robot_xy: np.ndarray, robot_heading: float, observations: Dict[str, Any]
    ):
        """更新VLFM风格的可视化显示"""
        try:
                ...
                if len(raw_frontiers) > 0 and hasattr(self, "_imu_offset"):
                    ...
                    for frontier in raw_frontiers:
                        offset_frontier = np.array(
                            [frontier[0] - self._imu_offset["x"], frontier[1] - self._imu_offset["y"]]
                        )
    ...
    def end_episode_recording(self, success: bool = False):
        ...
        if self.save_video:
            try:
                ...
                if (
                    self.visualizer.frame_count == 0
                    and self._current_rgb is not None
                ...
                if frames and len(frames) > 0:
                    # 计算episode指标
                    episode_duration = (
                        (rospy.Time.now() - self.episode_start_time).to_sec() if self.episode_start_time else 0
                    )
                    metrics = {"duration": episode_duration, "frames": len(frames), "success": 1.0 if success else 0.0}

                    # 保存视频
                    episode_id = f"{self.target_object}_{int(rospy.Time.now().to_sec())}"
                    video_path = self.video_recorder.save_episode_video(
                        frames=frames, episode_id=episode_id, metrics=metrics
                    )
