...
from vlfm.utils.jetracer_visualization import VLFMVisualizer, VLFMVideoRecorder, create_simple_visualization  # VLFM可视化工具
from omegaconf import DictConfig, OmegaConf                 # 配置管理库：DictConfig配置对象，OmegaConf配置工厂
    ...
    def _create_vlfm_observations(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.obstacle_map is not None:
                ...
                rospy.loginfo_throttle(10, f"状态: 探索中, 位置: ({robot_xy[0]:.2f}, {robot_xy[1]:.2f}), 前沿: {len(frontiers)}")
            else:
